...
namespace RBX_Alt_Manager.Forms
{
    public partial class AccountControl : Form
    {
        ...
        // Per-session shared secret for the Nexus control socket. Loopback clients don't need it — same trust
        // boundary as the app itself. A NON-loopback client (only reachable when AllowExternalConnections binds
        // every interface) MUST present it, so a LAN/internet peer can't drive the channel just by knowing a
        // public username. Regenerated every time the server starts.
        public static string NexusToken { get; private set; }

        // Read (TryGetValue) and written (Connect/Disconnect) from concurrent websocket-sharp session threads;
        // a plain Dictionary corrupts / spins under concurrent access, so use a thread-safe one.
        ...
        private void OpenServer()
        {
            ...
            NexusToken = Guid.NewGuid().ToString("N") + Guid.NewGuid().ToString("N"); // 256-bit per-session shared secret
            ...
            // websocket-sharp's sweep sends protocol-level Ping frames and force-closes any session that doesn't
            // return a Pong within WaitTime (1s). The in-game Lua client's liveness is an application-level JSON
            // "ping" text frame, not a protocol Pong, and many executor socket shims never auto-Pong — so the
            // sweep dropped healthy sessions at random. We have our own liveness (LastPing + AutoRelaunch).
            ...
            // (Removed Process.GetCurrentProcess().WaitForExit() — it pinned a ThreadPool thread for the entire
            //  lifetime of the app for nothing; WebSocketServer runs its own listener threads.)
        }

        public void EmitMessage(string Message, bool ToAll = false)
        {
            List<ControlledAccount> Targets = new List<ControlledAccount>();

            // EmitMessage can be reached from a websocket thread (the "Echo" command); enumerate the
            // ObjectListView on the UI thread into a snapshot, then send off-thread.
            this.InvokeIfRequired(() =>
            {
                foreach (ControlledAccount account in (ToAll ? AccountsView.Objects : AccountsView.CheckedObjects).Cast<ControlledAccount>())
                    Targets.Add(account);
            });
        ...
        public void LogMessage(string Message)
        {
            ...
            // Each log line creates a TextBox with a native window handle. Without a cap, a controlled client
            // spamming output accumulated tens of thousands of controls and exhausted the ~10k USER-handle
            // ceiling (CreateWindowEx then fails and the UI freezes). Evict the oldest beyond a fixed window.
            const int MaxOutputLines = 500;

            while (OutputPanel.Controls.Count > MaxOutputLines)
            {
                Control Oldest = OutputPanel.Controls[0];
                OutputPanel.Controls.RemoveAt(0);
                Oldest.Dispose();
            }
        ...
        private void AccountControl_Load(object sender, EventArgs e)
        {
            ...
            // Task.Wait wraps any startup fault (bad port, address already in use) in an AggregateException.
            // Catching only InvalidOperationException meant those failures were never surfaced — the socket
            // silently wasn't listening and the user had no idea why.
        ...
        private async void AutoRelaunchTimer_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (account.LinkedAccount == null) continue; // account was removed from the manager but is still linked here

                    if (account.AutoRelaunch)
                    {
                        // Presence is a class and is null before the first successful fetch — treat "no presence
                        // data" as not-in-game (so it relaunches) rather than NRE'ing the whole tick.
        ...
        private void copyJobIdToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (AccountsView.SelectedObject != null)
                try { string JobId = ((ControlledAccount)AccountsView.SelectedObject).InGameJobId; if (!string.IsNullOrEmpty(JobId)) Clipboard.SetText(JobId); } catch (Exception x) { Program.Logger.Warn($"[Clipboard] {x.Message}"); }
        }
        ...
        {
            foreach (Process p in Process.GetProcessesByName("RobloxPlayerBeta"))
                using (p) // dispose each Process handle; this fires on a repeating timer
                    Utilities.PostMessage(p.MainWindowHandle, 0x0112, 0xF020, 0);
        }
        ...
        private void CloseTimer_Tick(object sender, EventArgs e)
        {
            ...
            foreach (Process p in Processes) p.Dispose(); // release the handles enumerated each tick
        }
        ...
        {
            // GetProcesses() snapshots EVERY process on the machine each AutoRelaunch tick; dispose each so we
            // don't leak a full process-handle set (plus the MD5 object, now disposed) per tick.
