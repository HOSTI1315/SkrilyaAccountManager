...
namespace RBX_Alt_Manager.Nexus
{
    public class ControlledAccount
    {
        ...
        // connection's callbacks on its own thread, so two overlapping same-name reconnects could interleave the
        // check-then-act on Status/Context/ContextList and leak a zombie socket, leaving the account stuck
        // Offline while a live client kept pinging. Monitor is reentrant, so Connect()'s call to Disconnect() is fine.
        ...
        public void Connect(WebSocketContext Context)
        {
            lock (SyncLock)
            {
                ...
                // Connect runs on a websocket-sharp thread; RefreshObject touches the ObjectListView, so marshal it.
        ...
        public void Disconnect()
        {
            lock (SyncLock)
            {
                ...
                if (Context != null)
                {
                    ...
                    // Close the socket we're dropping. On a duplicate-name reconnect, Connect() calls Disconnect()
                    // on the still-open old socket; without this it leaks and its late OnClose later flips the
                    // (already reconnected) account Offline.
                    try { Context.WebSocket.Close(); } catch { }
                }

                // Disconnect runs on a websocket-sharp thread; marshal the ObjectListView update to the UI thread.
                AccountControl.Instance.InvokeIfRequired(() => AccountControl.Instance.AccountsView.RefreshObject(this));
            }
        }
        ...
        /// atomically under SyncLock so a concurrent Connect() can't swap Context between the check and the
        /// teardown. A superseded socket just drops its stale ContextList entry.
        /// </summary>
        ...
        public void HandleMessage(string Message)
        {
            ...
            if (Message.TryParseJson(out Command command))
            {
                ...
                {
                    // Reading a control's .Text must happen on the UI thread; this handler runs on a websocket thread.
                    string text = null;
                    AccountControl.Instance.InvokeIfRequired(() => text = AccountControl.Instance.GetTextFromElement(command.Payload["Name"]));
                    SendMessage($"ElementText:{text}");
                }
        ...
        public void SendMessage(string Message)
        {
            ...
            // Snapshot Context: a concurrent Disconnect (websocket thread) can flip Status / dispose the socket
            // between the check and the Send. Broadcasts (EmitMessage) used to crash the app on that race.
            WebSocketContext Ctx = Context;

            try { Ctx?.WebSocket?.Send(Message); }
            catch (Exception x) { Logger.Error($"SendMessage to {Username} failed: {x.Message}"); }
        }
    }
}