...
namespace RBX_Alt_Manager.Classes
{
    internal class AccountBrowser
    {
        ...
        // Cap concurrent browser launches: multi-selecting hundreds of accounts and hitting "Open Browser"
        // used to start every Chromium at once (memory / GDI / desktop-heap exhaustion -> freeze/OOM).
        // Windows still open, just in waves of this many.
        private static readonly System.Threading.SemaphoreSlim LaunchGate = new System.Threading.SemaphoreSlim(4);
        ...
        public async Task LaunchBrowser(string Url = "https://roblox.com/", Func<Task> PostPageCreation = null, Func<Page, Task> PostNavigation = null, string Script = null, string[] Arguments = null)
        {
            ...
            if (Position != null && Index >= 0 && ScreenGrid != null && ScreenGrid.Count > 0)
                Position = ScreenGrid[Index % ScreenGrid.Count]; // was % (Count - 1): DivideByZero at Count == 1, and the last cell was never used
            else
            ...
            finally { LaunchGate.Release(); } // release once this browser is up so the next wave can start while this one navigates
