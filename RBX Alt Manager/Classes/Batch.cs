...
namespace RBX_Alt_Manager.Classes
{
    internal class Batch
    {
        ...
        // The pending lists are appended from caller threads while the batch task drains them, and the two
        // dictionaries are written by the batch task while UI/renderer threads read them. Plain collections tear
        // their internal arrays under that access pattern and either crash or spin.
        ...
        private static async Task<JArray> DoBatchRequest()
        {
            ...
            while (PendingBatch.Count > 0)
            {
                ...
                // Take and remove under one lock — reading Count twice let a concurrent Add slip in between and
                // drop an entry, or shift the range so a request was sent twice.
                lock (PendingBatchLock)
                {
                ...
                if (!Response.IsSuccessful)
                    // was System.Web.HttpException — that type is .NET Framework only; HttpRequestException is
                    // the framework-provided equivalent and carries the status code.
                    throw new System.Net.Http.HttpRequestException($"{Response.StatusCode} Batch request failed\nBody: {Pending.ToArray()}\nError: {Response.ErrorMessage}");
            ...
            while (PendingPlace.Count > 0)
            {
                ...
                if (DetailsResponse.IsSuccessful)
                {
                    ...
                    foreach (GameDetails game in Games)
                    {
                        ...
                        PlaceDetails.TryAdd(PlaceId, game); // atomic check-and-add
