...
namespace RBX_Alt_Manager.Classes
{
    public static class Presence
    {
        ...
        {
            // An empty list is not a cheap request, it is a pointless one: the old caller hit this every two
            // minutes whenever the window was minimised, POSTing an empty body forever.
            ...
            {
                List<Account> Snapshot = AccountManager.GetAccountsSnapshot(); // avoid racing the live list on this background continuation

                foreach (var Presence in Data["userPresences"].ToObject<List<UserPresence>>())
                    if (Snapshot.FirstOrDefault(acc => acc.UserID == Presence.userId) is Account account)
                    {
