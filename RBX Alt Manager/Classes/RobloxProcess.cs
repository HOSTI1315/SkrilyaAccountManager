...
namespace RBX_Alt_Manager.Classes
{
    internal class RobloxProcess
    {
        ...
        private bool Exited; // guards the WaitForExitTimer teardown against running twice
        ...
        // Compiled once — these were being rebuilt for every log line on the 250ms ReadLogFile tick.
        ...
        // ConcurrentDictionary: UpdateMatches (a startup GitHub fetch) writes while ReadLogFile reads on the
        // 250ms timer. A plain Dictionary can tear its buckets under concurrent add+read and crash or spin.
        ...
        public RobloxProcess(Process process)
        {
            ...
            WaitForExitTimer.Elapsed += (s, e) =>
            {
                try
                {
                    // Inside the try, so a throw here (e.g. KillProcess disposing a not-yet-assigned LogStream)
                    // is logged instead of being swallowed by System.Timers.Timer — otherwise the client was
                    // never killed AND StreamDisposed stayed true, permanently disabling log watching.
                    ...
                    // Stop the timer and mark the stream disposed BEFORE disposing it, so an in-flight (or next
                    // scheduled) ReadLogFile tick bails out instead of touching a disposed stream.
                    StreamDisposed = true;
                    WaitForExitTimer?.Stop();

                    RobloxWatcher.LogFileRead -= ReadLogFile;

                    // This runs on the timer's ThreadPool thread; Seen/Instances are also touched by
                    // CheckProcesses on the UI thread, so guard the mutation with the shared lock.
                    lock (RobloxWatcher.CollectionsLock)
                    {
        ...
        private async Task WaitForLogPath()
        {
            ...
            // Dispose the spawned handle.exe process + its stdout pipe each attempt; this method retries up to
            // 31 times per account, so leaking them multiplied handle/pipe usage during mass launches.
            using (Process HandleProc = Process.Start(handle))
            {
                HandleProc.WaitForExit(6000);

                output = HandleProc.StandardOutput.ReadToEnd();
            }
