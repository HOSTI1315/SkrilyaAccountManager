...
### A. Job Objects — жёсткие лимиты CPU и RAM — ⏸ ОТЛОЖЕНО (решение 2026-08-08)

**Решение пользователя: пока не делаем.** Звёздочку «начинать с этого» снял — раздел ниже оставлен как есть,
но перед возвратом к нему прочитать две поправки:
...
Нерешённая развилка, с которой начинать разговор в следующий раз: лимит на каждый клиент отдельно или общий
бюджет на все (и `HARD_CAP` против `WEIGHT_BASED` по CPU — второй включается только при конкуренции за CPU).
...
- Бэкап FastFlags (`ClientSettings\ClientAppSettings.json`) и `GlobalBasicSettings_13.xml` до чистки, восстановление после.
...
**Стоит обсудить до реализации.** Это обёртка над платным сторонним сервисом:
работоспособность зависит от чужого API, нужен ключ и оплата, при закрытии сервиса фича
умирает. Ценность для нас под вопросом — оценить, нужно ли вообще, прежде чем писать код.
...
Поэтому первым шагом — не писать с нуля, а **сравнить**: возможно, botting это надстройка
над существующим AutoRelaunch (разделение на роли «бот»/«игрок» + групповой интервал +
массовое закрытие), а не отдельная подсистема. Скорее всего так и есть.
...
в `General/UiLanguage`. Старое окно остаётся английским: механизм ниже (.resx) до JS-текста не дотягивается,
поэтому для нового UI сделан свой.
...
**Фаза 2 (избранное + планировщик) не начата.**
...
Сделано: ~~H~~ (2026-08-04), ~~I~~, ~~J~~, ~~K~~, ~~L~~, ~~N~~, ~~G-новый UI~~ (все 2026-08-07).

1. ~~**M (паритет лаунчера)**~~ — ✅ сделано 2026-08-07.
2. ~~**A (Job Objects)**~~ — ⏸ отложено по решению пользователя 2026-08-08, см. раздел A.
3. **D (AFK)** — маленькая и самостоятельная; `PostMessageW` уже объявлен в `ClientWindows`.
4. ~~**Хвосты нового UI**~~ — ✅ сделано 2026-08-08, см. раздел O.
5. **B (изоляция, только очистка)** — заметная польза, средний объём.
6. **F (botting)** — начать с решения об архитектуре: релаунчеров теперь два (Nexus и раздел K),
   botting сделает третий.
7. **C (менеджер версий)** — самая большая; браться, когда остальное стабильно. Оценка устарела в нашу
   пользу: прямой запуск и обход папок версий уже сделаны в разделе I.
8. **G (i18n старого окна)** — только если старое окно решено оставить надолго; новый UI уже двуязычный.
9. **E (генератор)** — только если решено, что зависимость от платного стороннего API приемлема.
   Решение так и не записано; раздел H уже аргументирует против.
